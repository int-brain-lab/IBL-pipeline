...
CREATE TABLE `ibl_histology`.`_channel_brain_location` (
  ...
  CONSTRAINT `_channel_brain_location_ibfk_1` FOREIGN KEY (`subject_uuid`, `session_start_time`, `probe_idx`) REFERENCES `ibl_histology`.`_probe_trajectory` (`subject_uuid`, `session_start_time`, `probe_idx`) ON UPDATE CASCADE,
