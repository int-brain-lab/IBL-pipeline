...
class WheelMoveSet(dj.Imported):
    ...
    def make(self, key, one=None):
        ...
        try:  # Should be able to remove this
            ...
            all_loaded = all(
                [isinstance(wheel[lab], np.ndarray) for lab in wheel]
            ) and all(k in wheel for k in ("timestamps", "position"))
        ...
        moves = [
            dict(zip(keys, (i, on, off, vel_t[i], amp[i])), **move_key)
        ...
        for i, (on, off) in enumerate(on_off.reshape(-1, 2)):
            ...
            changes.extend(
                dict(change_key, move_id=i, change_id=j, change_time=t)
                for j, t in enumerate(ts[ind])
            )

        # Get the units of the position data
        units, *_ = infer_wheel_units(wheel.position)
        key["n_movements"] = wheel_moves["intervals"].shape[
            0
        ]  # total number of movements within the session
        key["total_displacement"] = float(
            np.diff(wheel.position[[0, -1]])
        )  # total displacement of the wheel during session
        key["total_distance"] = float(
            np.abs(np.diff(wheel.position)).sum()
        )  # total movement of the wheel
        key["n_direction_changes"] = sum(
            change_mask
...
class MovementTimes(dj.Computed):
    ...
    def make(self, key, one=None):
        ...
        start = np.nanmin(
            np.c_[trial_data["trial_stim_on_time"], trial_data["trial_go_cue_time"]],
            axis=1,
        )
        end = np.nanmin(
            np.c_[trial_data["trial_response_time"], trial_data["trial_feedback_time"]],
            axis=1,
        )
        ...
        assert ((start < end) | nan_trial).all() and (
            (np.diff(start) > 0) | np.isnan(np.diff(start))
        ).all(), "timestamps not increasing"
        ...
        onsets, final_movement, ids = extract_first_movement_times(
            wheel_move_data, trial_data, min_qt
        )
