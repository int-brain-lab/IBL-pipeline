...
from ibl_pipeline.ingest import ShadowIngestionError, acquisition, alyxraw, ephys
...
class ProbeTrajectoryTemp(dj.Imported):
    ...
    key_source = (
        (alyxraw.AlyxRaw - alyxraw.ProblematicData)
        & 'model="experiments.trajectoryestimate"'
    ).proj(probe_trajectory_uuid="uuid")
    ...
    def make(self, key):
        ...
        if ephys_real.ProbeInsertion & probe_insertion_key:
            subject_uuid, session_start_time, probe_idx = (
                ephys_real.ProbeInsertion & probe_insertion_key
            ).fetch1("subject_uuid", "session_start_time", "probe_idx")
        elif ephys.ProbeInsertion & probe_insertion_key:
            subject_uuid, session_start_time, probe_idx = (
                ephys.ProbeInsertion & probe_insertion_key
            ).fetch1("subject_uuid", "session_start_time", "probe_idx")
        else:
            raise ShadowIngestionError(
                "Non existing probe insertion: {}".format(probe_insertion_uuid)
            )
        ...
        if coord_uuid != "None":
            key["coordinate_system_uuid"] = (
                reference.CoordinateSystem & {"coordinate_system_uuid": coord_uuid}
            ).fetch1("coordinate_system_name")
    ...
    def create_entry(cls, key):
        ...
        if ProbeTrajectoryTemp & dict(probe_trajectory_uuid=probe_trajectory_uuid):
            subject_uuid, session_start_time, probe_idx, provenance = (
                ProbeTrajectoryTemp & dict(probe_trajectory_uuid=probe_trajectory_uuid)
            ).fetch1("subject_uuid", "session_start_time", "probe_idx", "provenance")
        else:
            raise ShadowIngestionError(
                "Non existing trajectory: {}".format(probe_trajectory_uuid)
            )

        brain_region_pk = grf(key, "brain_region")
        ontology, acronym = (
            reference.BrainRegion & dict(brain_region_pk=brain_region_pk)
        ).fetch1("ontology", "acronym")
