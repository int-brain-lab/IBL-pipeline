...
def get_date_range(subj, include_water_weight=True):
    ...
    if include_water_weight:
        ...
        if water_admin_range:
            first_water_admin_date, last_water_admin_date = water_admin_range.fetch1(
    ...
    if include_water_weight:
        ...
        last_date_array = [last_session_date, last_water_admin_date, last_weighing_date]
    else:
    ...
    first_date = np.min(first_date_array) - datetime.timedelta(days=3)

    last_date = np.max(last_date_array) + datetime.timedelta(days=3)

    first_date_str = first_date.strftime("%Y-%m-%d")
    last_date_str = last_date.strftime("%Y-%m-%d")

    date_array = [
        first_date + datetime.timedelta(days=day)
        for day in range(0, (last_date - first_date).days)
    ]

    # get Mondays
    mondays = [day.strftime("%Y-%m-%d") for day in date_array if day.weekday() == 0]
...
def get_status(subj):
    ...
    first_ephysrig = subj.aggr(
        behavior_ingest.Settings & 'pybpod_board like "%ephys%"',
        first_session="DATE(min(session_start_time))",
    )
    ...
    if len(first_trained_1a):
        first_trained_1a_date = first_trained_1a.fetch1("first_session").strftime(
            "%Y-%m-%d"
        )
        result.update(is_trained_1a=True, first_trained_1a_date=first_trained_1a_date)
    else:
        result.update(is_trained_1a=False)

    if len(first_trained_1b):
        first_trained_1b_date = first_trained_1b.fetch1("first_session").strftime(
            "%Y-%m-%d"
        )
        result.update(is_trained_1b=True, first_trained_1b_date=first_trained_1b_date)
    else:
    ...
    if len(first_ready4ephysrig):
        ...
        result.update(
            is_ready4ephysrig=True, first_ready4ephysrig_date=first_ready4ephysrig_date
        )
    else:
        result.update(is_ready4ephysrig=False)

    if len(first_ephysrig):
        first_ephysrig_date = first_ephysrig.fetch1("first_session").strftime(
            "%Y-%m-%d"
        )
        result.update(is_on_ephysrig=True, first_ephysrig_date=first_ephysrig_date)
    else:
        result.update(is_on_ephysrig=False)

    if len(first_ready4delay):
        first_ready4delay_date = first_ready4delay.fetch1("first_session").strftime(
            "%Y-%m-%d"
        )
        result.update(
            is_ready4delay=True, first_ready4delay_date=first_ready4delay_date
        )
    else:
    ...
    if len(first_ephys_session):
        first_ephys_session_date = first_ephys_session.fetch1("first_session").strftime(
            "%Y-%m-%d"
        )
        result.update(
            has_ephys_session=True, first_ephys_session_date=first_ephys_session_date
        )
    else:
...
def get_fit_pars(sessions):
    ...
    for session in sessions.fetch("KEY"):
        prob_left, threshold, bias, lapse_low, lapse_high = (sessions & session).fetch1(
...
def create_psych_curve_plot(sessions):
    ...
    for session in sessions.fetch("KEY"):
        ...
        ci = (
            smp.proportion_confint(
                n_trials_right, n_trials, alpha=0.032, method="normal"
            )
            - prob_right
        )
        ...
        behavior_data = go.Scatter(
            x=contrasts.tolist(),
            y=prob_right.tolist(),
            marker=dict(size=6, color=curve_color, line=dict(color="white", width=1)),
            mode="markers",
    ...
    layout = go.Layout(
        ...
        yaxis=dict(title="Probability choosing right", range=[-0.05, 1.05]),
...
def create_rt_contrast_plot(sessions):
    ...
    for session in sessions.fetch("KEY"):
        contrasts, prob_left, reaction_time, ci_low, ci_high = (
            ...
            "signed_contrasts",
            "prob_left",
            "reaction_time_contrast",
            "reaction_time_ci_low",
            "reaction_time_ci_high",
        )
    ...
    layout = go.Layout(
        width=630,
        height=350,
        title=dict(text="Reaction time - Contrast", x=0.25, y=0.85),
        xaxis=dict(title="Contrast (%)"),
        yaxis=dict(title="Reaction time (s)"),
        legend=dict(x=1.1, y=0.9, orientation="v"),
        template=dict(layout=dict(plot_bgcolor="white")),
    )
...
def create_rt_trialnum_plot(trials):
    ...
    trials_go_cue_only = (
        trials & "trial_stim_on_time is NULL and trial_go_cue_trigger_time is not NULL"
    )
    trials_stim_on = trials & "trial_stim_on_time is not NULL"
    ...
    layout = go.Layout(
        width=630,
        height=400,
        title=dict(text="Reaction time - trial number", x=0.26, y=0.85),
...
def create_status_plot(
    ...
    if status["is_trained_1a"]:
        data.append(
            go.Scatter(
                x=[status["first_trained_1a_date"], status["first_trained_1a_date"]],
                y=yrange,
                mode="lines",
    ...
    if status["is_trained_1b"] and (not public):
        data.append(
            go.Scatter(
                x=[status["first_trained_1b_date"], status["first_trained_1b_date"]],
                y=yrange,
                mode="lines",
    ...
    if status["is_on_ephysrig"] and (not public):
        data.append(
            go.Scatter(
                x=[status["first_ephysrig_date"], status["first_ephysrig_date"]],
                y=yrange,
                mode="lines",
    ...
    if status["is_ready4delay"] and (not public):
        data.append(
            go.Scatter(
                x=[status["first_ready4delay_date"], status["first_ready4delay_date"]],
                y=yrange,
                mode="lines",
    ...
    if status["is_over_seven_months"]:
        data.append(
            go.Scatter(
                x=[status["seven_months_date"], status["seven_months_date"]],
                y=yrange,
                mode="lines",
    ...
    if status["has_ephys_session"]:
        data.append(
            go.Scatter(
                ...
                y=yrange,
                mode="lines",
                line=dict(width=2, color="rgba(5, 142, 255, 1)", dash="dashdot"),
...
def create_monday_plot(
    data, yrange, mondays, xaxis="x1", yaxis="y1", show_legend_external=True
):
    ...
    for imonday, monday in enumerate(mondays):
        ...
        data.append(
            go.Scatter(
                ...
                y=yrange,
                mode="lines",
                line=dict(width=0.5, color="gray", dash="dot"),
                name="Mondays",
                xaxis=xaxis,
                yaxis=yaxis,
...
def create_good_enough_brainmap_plot(
    data,
    yrange,
    ephys_dates,
    good_enough,
    ...
    shown_red = 0
    shown_blue = 0
    for i_good_date, (ephys_date, good) in enumerate(zip(ephys_dates, good_enough)):
