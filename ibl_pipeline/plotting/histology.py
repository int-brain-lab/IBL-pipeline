...
from ibl_pipeline import acquisition, ephys, histology, mode, one, subject
...
class SubjectSpinningBrain(dj.Imported):
    ...
    def make(self, key):
        ...
        fig = GifFigure(hplt.generate_spinning_brain_frames, trajs)

        fig_link = path.join(
            root_path, "subject_spinning_brain", str(key["subject_uuid"]) + ".gif"
        )
