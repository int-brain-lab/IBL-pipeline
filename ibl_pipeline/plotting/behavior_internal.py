...
class WaterTypeColor(dj.Computed):
    ...
    def make(self, key):
        ...
        if key["watertype_name"] in original_water_types:
            ...
            water_type_color = dict(
                **key,
                water_type_color="rgba({}, {}, {}, 0.7)".format(
        ...
        if action.WaterAdministration * action.Weighing & key:
            ...
            data.append(
                go.Scatter(
                    ...
                    name="Weight",
                    marker=dict(
                        size=6, color="black", line=dict(color="white", width=1)
                    ),
                    legendgroup="weight",
                    yaxis="y2",
                )
            ...
            for iwater, water_res in enumerate(water_restrictions.fetch(as_dict=True)):
                ...
                data.append(
                    go.Scatter(
                        ...
                        mode="lines",
                        line=dict(width=1, color="orange", dash="dashdot"),
                ...
                data.append(
                    go.Scatter(
                        ...
                        mode="lines",
                        line=dict(width=1, color="red", dash="dashdot"),
            ...
            layout = go.Layout(
                yaxis=dict(title="Water intake (mL)", range=yrange_water),
                yaxis2=dict(
                    title="Weight (g)",
                    overlaying="y",
                    side="right",
                ...
                width=1000,
                height=500,
                title=dict(text="Water intake and weight", x=0.3, y=0.9),
                xaxis=dict(
                    title="Date", range=[d["first_date_str"], d["last_date_str"]]
                ),
...
if "0.12" in dj.__version__:
    ...
    raise NotImplementedError(
        "Cannot declare WaterWeight table without DataJoint 0.12 or 0.13."
    )


ingested_sessions = (
    acquisition.Session & "task_protocol is not NULL" & behavior_ingest.TrialSet
)
subjects_alive = (
...
class DailyLabSummary(dj.Computed):
    ...
    def make(self, key):
        ...
        last_sessions = (
            last_sessions * acquisition.Session * behavior.SessionTrainingStatus
        )
        ...
        for entry in summary:
            ...
            if not len(acquisition.Session & subj):
                ...
                last_date = last_session_date.fetch1("session_date").strftime(
                    "%Y-%m-%d"
                )
                ...
                if not len(behavior_ingest.CompleteTrialSession & last_session):
                    ...
                    behavior_shared.CumulativeSummary
                    & last_session.proj(latest_session="session_date")
                ):
                    data_update_status = """
    ...
    def detect_dead_subjects_from_alyx(cls, insert_into_death_table=False):
        ...
        for subj in tqdm(cls.SubjectSummary.fetch("KEY")):
            ...
            dead_subj = one.alyx.rest(
                "subjects", "list", id=str(subj["subject_uuid"]), alive=False
            )
            if dead_subj:
                dead_subj_uuids.append(subj["subject_uuid"])
                if insert_into_death_table:
                    subject.Death.insert1(
                        dict(
