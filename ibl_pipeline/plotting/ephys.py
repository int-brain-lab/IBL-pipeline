...
class ValidAlignSort(dj.Lookup):
    ...
    contents = [
        ...
        [
            ...
            "",
            "movement_onset - trial_stim_on_time",
            "trial_stim_on_time - movement_onset",
            "stim on",
        ],
        [
            "movement",
...
class RasterLayoutTemplate(dj.Lookup):
    ...
    layout = go.Layout(
        ...
        width=580,
        height=370,
        margin=go.layout.Margin(l=50, r=30, b=40, t=80, pad=0),
        title=dict(text="Raster, aligned to ", x=0.21, y=0.87),  # add align_event here
        xaxis=dict(title="Time (sec)", range=[-1, 1], showgrid=False),
        yaxis=dict(title="Trial idx", showgrid=False),
    )
    ...
    template_0 = dict(
        template_idx=0,
        raster_data_template=go.Figure(data=data0, layout=layout).to_plotly_json(),
    )
    ...
    template_2 = dict(template_idx=2, raster_data_template=fig.to_plotly_json())
    ...
    template_3 = dict(
        template_idx=3,
        raster_data_template=go.Figure(data=data3, layout=layout).to_plotly_json(),
    )
...
class Raster(dj.Computed):
    ...
    key_source = (
        ephys.DefaultCluster * ValidAlignSort
        & behavior.TrialSet
        & ephys.AlignedTrialSpikes
        & [{"wheel_needed": 0}, wheel.MovementTimes]
    )
    ...
    def plot_regular(trials, key, ax, x_lim=[-1, 1]):
        ...
        spk_trial_ids = np.hstack(
            [[trial_id] * len(spk_time) for trial_id, spk_time in enumerate(spk_times)]
        )

        ax.plot(np.hstack(spk_times), spk_trial_ids, "k.", alpha=0.5, markeredgewidth=0)
        ...
        if key["sort_by"] != "trial_id":
            ...
            for i, ind in enumerate(u_inds[:-1]):
                ax.fill_between(
                    [-1, 1], u_inds[i], u_inds[i + 1] - 1, color=colors[i], alpha=alpha
                )
    ...
    def plot_difference(trials, key, ax, x_lim=[-1, 1]):
        ...
        sorting_variable, mark_variable, label_variable = (ValidAlignSort & key).fetch1(
            "sorting_variable", "mark_variable", "label_variable"
        )

        trials_left = (
            trials & 'trial_response_choice="CW"' & "trial_signed_contrast < 0"
        )
        trials_right = (
            trials & 'trial_response_choice="CCW"' & "trial_signed_contrast > 0"
        )
        ...
        for trial_group in trial_groups:
            ...
            spk_times, marking_points = (
                trial_group["trials"].proj(
                    "trial_spike_times", sort=sorting_variable, mark_point=mark_variable
                )
                & key
            ).fetch("trial_spike_times", "mark_point", order_by="sort")
    ...
    def make(self, key):
        ...
        if wheel_needed:
            trials = (trials * wheel.MovementTimes).proj(..., "movement_onset")
...
class PsthTemplate(dj.Lookup):
    ...
    layout = go.Layout(
        ...
        yaxis=dict(title="Firing rate (spks/sec)", showgrid=False),
    )
    ...
    contents = [
        dict(
            ...
            psth_data_template=go.Figure(
                data=[left, right, incorrect, all], layout=layout
            ).to_plotly_json(),
        ),
        dict(
            psth_template_idx=1,
...
class Psth(dj.Computed):
    ...
    def make(self, key):
        trials_all = (
            (behavior.TrialSet.Trial * ephys.AlignedTrialSpikes & key).proj(
                "trial_start_time",
                "trial_stim_on_time",
                "trial_response_time",
                "trial_feedback_time",
        ...
        trials_left = (
            trials_all & 'trial_response_choice="CW"' & "trial_signed_contrast < 0"
        )
        trials_right = (
            trials_all & 'trial_response_choice="CCW"' & "trial_signed_contrast > 0"
        )
        ...
        if len(trials_right):
            ...
            entry.update(
                ...
                psth_right_upper=",".join(
                    "{:0.5f}".format(x) for x in psth_right_upper
                ),
                psth_right_lower=",".join(
                    "{:0.5f}".format(x) for x in psth_right_lower
                ),
            )
        ...
        if len(trials_incorrect):
            (
                _,
                psth_incorrect,
                psth_incorrect_upper,
                psth_incorrect_lower,
            ) = putils.compute_psth_with_errorbar(
                trials_incorrect, "incorrect", align_event, as_plotly_obj=False
            )
            entry.update(
                psth_incorrect=",".join("{:0.5f}".format(x) for x in psth_incorrect),
                psth_incorrect_upper=",".join(
                    "{:0.5f}".format(x) for x in psth_incorrect_upper
                ),
                psth_incorrect_lower=",".join(
                    "{:0.5f}".format(x) for x in psth_incorrect_lower
                ),
            )
...
class DepthRasterTemplate(dj.Lookup):
    ...
    first_trial_mark = go.Scatter(
        ...
        mode="lines",
        line=dict(color="rgba(20, 40, 255, 1)", width=1.5),
    ...
    last_trial_mark = go.Scatter(
        ...
        mode="lines",
        line=dict(color="rgba(255, 20, 20, 1)", width=1.5),
    ...
    layout1 = go.Layout(
        ...
        height=900,
        margin=go.layout.Margin(l=50, r=30, b=40, t=80, pad=0),
        title=dict(text="Depth raster of the entire session", x=0.45, y=0.95),
        xaxis=dict(
            title="Time (sec)",
    ...
    trial_stim_on_mark = go.Scatter(
        ...
        mode="lines",
        line=dict(color="rgba(20, 40, 255, 0.4)", width=1.5),
        name="Stim on",
    )
    ...
    trial_movement_mark = go.Scatter(
        ...
        # y=plot_ylim, # fetched from DepthRasterPerTrial
        mode="lines",
        line=dict(color="rgba(200, 40, 25, 0.4)", width=1.5),
        name="Movement",
    )
    ...
    trial_feedback_mark = go.Scatter(  # if null, skip this line
        ...
        # y=plot_ylim, # fetched from DepthRasterPerTrial
        mode="lines",
        line=dict(color="rgba(60, 255, 10, 0.4)", width=1.5),
        name="Feedback",
    )
    ...
    trial_stim_off_mark = go.Scatter(
        ...
        # y=plot_ylim, # fetched from DepthRasterPerTrial
        mode="lines",
        line=dict(color="rgba(230, 190, 20, 0.8)", width=1.5),
        name="Stim off",
    )
...
class DepthRaster(dj.Computed):
    ...
    def make(self, key):
        ...
        key["plot_xlim"], key["plot_ylim"] = putils.create_driftmap_plot(
            spikes_data, fig_dir=fig_link_full, store_type="s3"
        )

        putils.create_driftmap_plot(
            spikes_data, dpi=25, fig_dir=fig_link_low, store_type="s3"
        )

        putils.create_driftmap_plot(
            spikes_data, dpi=10, fig_dir=fig_link_very_low, store_type="s3"
        )
        ...
        if len(trials):
            ...
            last_end = (
                ...
                .proj(
                    session_end_relative_to_start="session_end_time - session_start_time"
                )
                .fetch1("session_end_relative_to_start")
            )
...
class DepthRasterExampleTrial(dj.Computed):
    ...
    def _get_trial_type(self, trial):
        ...
        if trial["trial_response_choice"] == "CW" and trial["trial_feedback_type"] == 1:
            return "Correct Left Contrast"

        elif (
            trial["trial_response_choice"] == "CCW"
            and trial["trial_feedback_type"] == 1
        ):
    ...
    def make(self, key):
        ...
        trials_all = (
            (behavior.TrialSet.Trial * wheel.MovementTimes & key).proj(
                "trial_response_choice",
                "trial_feedback_type",
                "trial_stim_on_time",
                "movement_onset",
        ...
        if mode_ == "example":
            ...
            for contrast in tqdm(
                dj.U("trial_signed_contrast") & trials_all, position=0
            ):
        ...
        try:
            ...
            print(
                "Failed to insert all trials at once, \
...
class DepthPethTemplate(dj.Lookup):
    ...
    layout = go.Layout(
        images=[
            dict(
                source="",  # to be replaced by s3 link in Depth
    ...
    contents = [
        dict(
            ...
            depth_peth_template=go.Figure(data=data, layout=layout).to_plotly_json(),
        )
    ]
...
class DepthPeth(dj.Computed):
    ...
    def make(self, key):
        normed_peth, depths, time = (
            ephys_analyses.DepthPeth * ephys_analyses.NormedDepthPeth & key
        ).fetch1("normed_peth", "depth_bin_centers", "time_bin_centers")

        peth_df = pd.DataFrame(
            normed_peth, columns=np.round(time, decimals=2), index=depths.astype("int")
        )
        ...
        fig = PngFigure(
            eplt.depth_peth,
            dict(peth_df=peth_df),
            dict(colors=rb_cmap.as_matplotlib(), as_background=True, return_lims=True),
        )
...
class SpikeAmpTimeTemplate(dj.Lookup):
    ...
    layout = go.Layout(
        ...
        width=580,
        height=400,
        title=dict(text="Spike amp - time", x=0.5, y=0.85),
        legend=dict(x=1.2, y=0.8, orientation="v"),
        template=dict(layout=dict(plot_bgcolor="white")),
    )
...
class SpikeAmpTime(dj.Computed):
    ...
    def make(self, key):
        ...
        ).fetch("KEY", "cluster_spikes_times", "cluster_spikes_amps")

        for ikey, spike_times, spike_amps in tqdm(
            zip(keys, clusters_spike_times, clusters_spike_amps), position=0
        ):
            fig = PngFigure(
                eplt.spike_amp_time,
                data=dict(spike_times=spike_times, spike_amps=spike_amps * 1e6),
                ax_kwargs=dict(s=8, as_background=True, return_lims=True),
                dpi=100,
                figsize=[10, 5],
            )
...
class AutoCorrelogramTemplate(dj.Lookup):
    ...
    data = dict(
        ...
        name="data",
        type="scatter",
        marker=dict(color="rgba(51, 76.5, 204, 0.8)"),
        x0=0,
    )
    ...
    layout = dict(
        width=580,
        height=400,
        title=dict(text="Autocorrelogram", x=0.5, y=0.85),
        xaxis=dict(
            title="Lag (ms)",
            showgrid=False,
            linecolor="lightgray",
    ...
    contents = [
        dict(
            acg_template_idx=0,
            acg_template=go.Figure(data=data, layout=layout).to_plotly_json(),
        )
    ]
...
class AutoCorrelogram(dj.Computed):
    ...
    def _acorr(self, spike_times, bin_size=None, window_size=None):
        ...
        xc = population.xcorr(
            spike_times,
            np.zeros_like(spike_times).astype("int"),
            bin_size=bin_size,
    ...
    def make(self, key):
        ...
        spike_times = (ephys.DefaultCluster & key).fetch1("cluster_spikes_times")
...
class WaveformTemplate(dj.Lookup):
    ...
    layout = go.Layout(
        images=[
            dict(
                source="",  # replace with plotting_fig_link from Waveform
                #  sizex=plot_xlim[1] - plot_xlim[0],
                #  sizey=plot_ylim[1] - plot_ylim[0],
                #  x=plot_xlim[0],
                #  y=plot_ylim[1],
        ...
        width=580,
        height=400,
        title=dict(text="Template waveforms", x=0.55, y=0.85),
        margin=go.layout.Margin(l=100, r=30, b=40, t=80, pad=0),
        legend=dict(x=1.2, y=0.8, orientation="v"),
        yaxis=dict(
    ...
    contents = [
        dict(
            ...
            waveform_template=go.Figure(data=axis, layout=layout).to_plotly_json(),
        )
    ]
...
class Waveform(dj.Computed):
    ...
    def make(self, key):
        ...
        keys, clusters_waveforms, clusters_waveforms_channels = (
            ephys.DefaultCluster() & key
        ).fetch("KEY", "cluster_waveforms", "cluster_waveforms_channels")

        for ikey, waveforms, waveforms_channels in tqdm(
            zip(keys, clusters_waveforms, clusters_waveforms_channels), position=0
        ):
