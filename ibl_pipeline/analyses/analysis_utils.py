...
def compute_psych_pars(trials):
    ...
    q = dj.U("signed_contrast").aggr(
        trials, n="count(*)", n_right='sum(trial_response_choice="CCW")'
    )
...
def compute_performance_easy(trials):
    ...
    if not len(trials_easy):
        ...
        trials_response_choice, trials_signed_contrast = trials_easy.fetch(
            "trial_response_choice", "signed_contrast"
        )
        n_correct_trials_easy = np.sum(
            (trials_response_choice == "CCW") & (trials_signed_contrast > 0)
        ) + np.sum((trials_response_choice == "CW") & (trials_signed_contrast < 0))
...
def compute_reaction_time(trials, compute_ci=False):
    ...
    if compute_ci:
        ci_rt = grouped_rt.apply(lambda x: bootstrap.ci(x, scipy.nanmedian, alpha=0.32))
        ci_low = np.array([x[0] if not np.isnan(x[0]) else None for x in ci_rt])
        ci_high = np.array([x[1] if not np.isnan(x[1]) else None for x in ci_rt])
