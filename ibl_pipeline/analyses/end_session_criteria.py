...
def session_end_indices(trials, make_plot=False, ax=None):
    ...
    trials.loc[np.abs(trials["signed_contrast"]) < 0.5, "correct_easy"] = np.NaN
    trials["n_trials_last_5"] = (
        trials["trial_start_time"]
        .expanding()
        .apply(lambda x: sum((x[-1] - x[0:-1]) < 5 * 60), raw=True)
    )
    ...
    trials["perf_local"] = (
        trials["correct"]
        .rolling(perf_win_size)
        .apply(lambda x: sum(x) / x.size, raw=True)
    )
    trials["perf_global"] = (
        trials["correct"].expanding().apply(lambda x: sum(x) / x.size, raw=True)
    )
    trials["perf_delta"] = (trials["perf_global"] - trials["perf_local"]) / trials[
        "perf_global"
    ]

    # Performance for easy trials only
    def last(x):
        return x[~np.isnan(x)][-perf_win_size:]  # Find last n values that aren't nan

    trials["perf_local_ez"] = (
        trials["correct_easy"]
        .expanding()
        .apply(
            lambda x: sum(last(x)) / last(x).size if last(x).size else np.nan, raw=True
        )
    )
    trials["perf_global_ez"] = (
        trials["correct_easy"]
        .expanding()
        .apply(lambda x: (sum(x == 1) / sum(~np.isnan(x))), raw=True)
    )
    trials["perf_delta_ez"] = (
        trials["perf_global_ez"] - trials["perf_local_ez"]
    ) / trials["perf_global_ez"]

    status_idx = dict.fromkeys(EndCriteria.contents)
    status_idx["long_rt"] = (
        (trials.RT_delta & (trials.index > min_trials)).idxmax()
        if (trials.RT_delta & (trials.index > min_trials)).any()
        else np.nan
    )
    status_idx["perf_ez<40"] = (
        (trials["perf_delta_ez"] > 0.4) & (trials.index > min_trials)
    ).idxmax()
    status_idx["perf<40"] = (
        (trials["perf_delta_ez"] > 0.4) & (trials.index > min_trials)
    ).idxmax()
    status_idx["<400_trials"] = (
        (trials.trial_start_time > 45 * 60) & (trials.index < min_trials)
    ).idxmax()
    status_idx[">45_min_&_stopped"] = (
        (trials.trial_start_time > 45 * 60) & (trials["n_trials_last_5"] < 45)
    ).idxmax()
    ...
    if make_plot:
        ...
        trials.plot(
            y=["RT_local", "RT_global", "perf_local_ez", "perf_global_ez"],
            ax=ax,
            color=c,
        )
        ax.set_ylim([0, 2])
...
class SessionEndCriteriaImplemented(dj.Computed):
    ...
    def make(self, key):
        ...
        exclude = [
            ...
            "perf<40",
            "perf_ez<40",
        ]  # List of unimplemented criteria
        status_idx = {
            k: v for (k, v) in status_idx.items() if k not in exclude
        }  # Remove from dict
