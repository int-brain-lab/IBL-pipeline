...
subjects = pd.DataFrame.from_dict(((subject.Subject() - subject.Death()) & 
    action.WaterRestriction().proj()).fetch(as_dict=True, order_by='subject_birth_date'))
...
for i, mouse in enumerate(subjects['subject_nickname']):
    ...
    try:
        ...
        fig.savefig(os.path.join(path + '%s_overview.pdf'%mouse))
        plt.close(fig)
        print(os.path.join(path + '%s_overview.pdf'%mouse))
