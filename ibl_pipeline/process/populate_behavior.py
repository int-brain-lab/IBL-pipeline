    ...
    **kwargs,
    ...
    while (
        ...
        data.DataSet.insert_with_alyx_rest(
            uuids,
            behavior.CompleteTrialSession.required_datasets
            + behavior.CompleteTrialSession.other_datasets,
        )
        ...
        with dj.config(safemode=False):
            (
                ...
                & behavior_plotting.CumulativeSummary.get_outdated_entries().fetch(
                    "KEY"
                )
            ).delete()
        ...
        behavior_plotting.SubjectLatestDate.insert(
            subject_latest_date, skip_duplicates=True
        )

        need_update = (
            behavior_plotting.SubjectLatestDate.proj(inserted_date="latest_date")
            * subject_latest_date
