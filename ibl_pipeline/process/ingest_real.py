    ...
    if fresh:
        ...
        if use_uuid:
            ...
            if len(pk) == 1 and "uuid" in pk[0]:
                q_insert = q_src_table - (
                    dj.U(pk[0]) & target_table & f"{pk[0]} is not null"
                )
            else:
        ...
        for parent_table, parent_fk_info in target_table.parents(
            ...
            if np.all(
                [
                    target_table.heading.attributes[attr].nullable
            ...
            if dj.utils.to_camel_case(
                parent_table.full_table_name.split(".")[-1].replace("`", "")
            ) in ("BrainRegion"):
                continue
