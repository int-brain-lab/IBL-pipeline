...
def update_shadow_field(field):
    ...
    for key in tqdm(keys, position=0):
        ...
        dj.Table._update(
            reference_ingest.BrainRegion & dict(brain_region_pk=key["pk"]),
            field,
            field_value,
        )
...
def update_real_field(field):
    ...
    entries_for_updates = reference.BrainRegion.proj(
        "brain_region_pk", field_real=field
