...
class BrainAtlas:
    def __init__(self):
        ...
        for acronym, name in zip(
            *reference.BrainRegion.fetch("acronym", "brain_region_name")
        ):
            self.name_lookup.update({acronym: name})

        self.atlas_dict = dict()
        for level in (dj.U("brain_region_level") & reference.BrainRegion).fetch(
            "KEY", order_by="brain_region_level"
        ):
            for acronym, name in zip(
                *(reference.BrainRegion & level).fetch(
                    "acronym", "brain_region_name", order_by="graph_order"
                )
            ):
                if level == 0:
    ...
    def get_parents(acronym):
        ...
        while reference.ParentRegion & dict(acronym=acronym):
            acronym = (reference.ParentRegion & dict(acronym=acronym)).fetch1("parent")
