...
from ibl_pipeline import acquisition, reference, subject, update
...
def get_user(key):
    if len(acquisition.SessionUser & key) == 1:
        user = (acquisition.SessionUser & key).fetch1("user_name")
    elif len(subject.SubjectUser & key) == 1:
        user = (subject.SubjectUser & key).fetch1("responsible_user")
    elif len(reference.LabMember & key) == 1:
        user = (reference.LabMember & key).fetch1("user_name")
    else:
        user = None
    return user
    ...
    for t in tablenames:
        ...
        for deleted_key in (real - shadow.proj(*real.primary_key)).fetch("KEY"):
            ...
            deleted_record = dict(
                ...
                original_ts=deleted_entry[
                    [key for key in deleted_entry.keys() if "_ts" in key][0]
                ],
                pk_dict=deleted_key,
            ...
            if user:
                email = (reference.LabMember & 'user_name="{}"'.format(user)).fetch1(
                    "email"
                )
                deleted_record.update(responsible_user=user, user_email=email)
        ...
        shadow_records = (shadow & common_records).fetch(order_by=kstr, as_dict=True)
        real_records = (real & common_records.proj()).fetch(order_by=kstr, as_dict=True)
        real_keys = (real & common_records.proj()).fetch("KEY", order_by=kstr)

        for shadow_record, real_record, pk in zip(
            shadow_records, real_records, real_keys
        ):
            ...
            for attr in shadow_record:
                ...
                if shadow_value != real_value and (
                    isinstance(shadow_value, str) and "_ts" not in shadow_value
                ):
                    ...
                    update_record = dict(
                        ...
                        original_ts=real_record[
                            [key for key in real_record.keys() if "_ts" in key][0]
                        ],
                        update_ts=shadow_record[
                            [key for key in shadow_record.keys() if "_ts" in key][0]
                        ],
                        pk_dict=pk,
                    ...
                    user = get_user(pk)
                    if user:
                        update_record.update(responsible_user=user, user_email=email)
