...
class Table(dj.Lookup):
                ...
                self.insert1(
                    dict(
                        **table_key,
                        ...
                        table_label="auto"
                        if issubclass(table_obj, (dj.Imported, dj.Computed))
                        else "part",
                        table_parent=eval(
                            re.match("(^.*)\..*$", table).group(1)
                        ).full_table_name
                        if issubclass(table_obj, dj.Part)
                        else None
                    )
                )
    ...
    def _insert_virtual_tables(self):
        ...
        virtuals = (
            Graph(behavior.TrialSet()).get_table_list(virtual_only=True)
            + Graph(ephys.DefaultCluster()).get_table_list(virtual_only=True)
            + Graph(behavior_analyses.BehavioralSummaryByDate()).get_table_list(
                virtual_only=True
            )
        )
    ...
    def insert_tables(self, table_type="All"):
        ...
        if table_type == "session":
            ...
            ValueError(
                "Invalid table_type. It has to be one of the following: session, date, virtual"
            )


table_kwargs = dict(order_by="table_order desc", as_dict=True)
populate_kwargs = dict(
    suppress_errors=True, display_progress=True, return_exception_objects=True
)
...
class Run(dj.Manual):
    ...
    def _delete_table(self, t, key, table_type="session", save_status=True):
        ...
        if t["full_table_name"] == "`ibl_ephys`.`__aligned_trial_spikes`":
            for cluster in tqdm((ephys.DefaultCluster & key).fetch("KEY"), position=0):
                (table_class & cluster).delete_quick()
        else:
            (table_class & key_del).delete_quick()

        if save_status:
            dj.Table._update(RunStatus.TableStatus & key_table, "status", "Deleted")
            dj.Table._update(
    ...
    def make(self, key):
        ...
        tables_session = (Table & 'table_order_category="session"').fetch(
            **table_kwargs
        )
        tables_date = (Table & 'table_order_category="date"').fetch(**table_kwargs)
        tables_virtual = (Table & 'table_order_category="virtual"').fetch(
            **table_kwargs
        )

        # start this job
        if not RunStatus & key:
            RunStatus.insert1(dict(**key, run_start_time=datetime.datetime.now()))
        else:
            dj.Table._update(
                RunStatus & key, "run_restart_time", datetime.datetime.now()
            )
        ...
        for t in tables_session[::-1] + tables_date[::-1]:
            ...
            if t["table_label"] == "auto":
                ...
                dj.Table._update(status, "populate_start_time", datetime.datetime.now())

                if (not (table_class.key_source - table_class.proj()) & key_pop) and (
                    not RunStatus.TableStatus & table_key & 'status="Success"'
                ):
                    dj.Table._update(status, "status", "Error")
                    dj.Table._update(
                        status, "populate_done_time", datetime.datetime.now()
                    )
                    dj.Table._update(status, "error_message", "No tuples to populate")
                else:
                    ...
                    if errors:
                        ...
                        if len(errors) > 10:
                            errors = errors[:10]
                        dj.Table._update(status, "error_message", str(errors))
                    else:
                        dj.Table._update(status, "status", "Success")
                        dj.Table._update(status, "error_message", "")
                        # mark its part table to Success
                        for part_table in (
                            Table & dict(table_parent=t["full_table_name"])
                        ).fetch("KEY"):
                            dj.Table._update(
        ...
        # end this job
        dj.Table._update(RunStatus & key, "run_end_time", datetime.datetime.now())

        if (
            RunStatus.TableStatus
            & key
            & [tables_session, tables_date]
            & 'status in ("Success")'
        ):
            job_status = "Partial Success"
            if (
                not RunStatus.TableStatus
                & key
                & [tables_session, tables_date]
...
if __name__ == "__main__":
    ...
    session = acquisition.Session & {
        "session_uuid": "f8d5c8b0-b931-4151-b86c-c471e2e80e5d"
    }
    ...
    Session.insert1(
        dict(**entry[0], job_date=datetime.datetime.now().date()), skip_duplicates=True
    )
