...
def plot_contrast_heatmap(mouse, lab, ax, xlims):
    ...
    # now heatmap
    sns.heatmap(pp2, linewidths=0.5, ax=ax, vmin=0, vmax=1, cmap=cmap, cbar=True,
...
def plot_psychometric(df, color='black', ax=None, **kwargs):
    ...
    if not brokenXaxis:
        ...
        ax.set_xticklabels(['-100', '-50', '0', '50', '100'], size='x-small', rotation=45)
        ax.set_xlim([-110, 110])
    else:
