...
from ibl_pipeline import acquisition, action, behavior, data, reference, subject  # noqa
from ibl_pipeline.analyses import behavior as behavior_analysis
from ibl_pipeline.utils import (
    psychofit as psy,  # https://github.com/cortex-lab/psychofit
)
...
def plot_water_weight_curve(weight_water, baseline, ax, xlims):
    ...
    wa_unstacked = weight_water.pivot_table(
        index="days", columns="water_type", values="water_administered", aggfunc="sum"
    ).reset_index()
    ...
    wa_unstacked = wa_unstacked.reindex(
        columns=["days", "Wa 10% Sucr", "Wa", "Wa 2% CA", "Hdrg", "Wa 15% Sucr"]
    )
    ...
    plotvar = plotvar.reindex(
        np.arange(weight_water.days.min(), weight_water.days.max() + 1)
    )
    ...
    # plot weight curve
    sns.lineplot(
        x=weight_water2.days, y=weight_water2.weight, ax=righty, color=".15", marker="o"
    )

    # show the start of each water restriction
    sns.scatterplot(
        x=baseline.day_start,
        y=baseline.reference_weight,
        ax=righty,
        marker="D",
        facecolor="white",
        edgecolor="black",
    ...
    for d in range(len(baseline)):
        ...
        righty.plot(
            (baseline.day_start[d], baseline.day_end[d]),
    ...
    if (weight_water.days.max() - weight_water.days.min()) > 6:
        ...
        ax.set_xticklabels(
            [
                weight_water.date[i].strftime("%b-%d")
                for i, dt in enumerate(weight_water.date)
                if dt.weekday() == 0
            ]
        )
...
def plot_trialcounts_sessionlength(mouse, lab, ax, xlims):
    ...
    sns.lineplot(
        x="session_date", y="n_trials", marker="o", color=".15", data=n_trials, ax=ax
    )
    ...
    duration = pd.DataFrame(
        (
            acquisition.Session * subject.Subject * subject.SubjectLab
            & 'subject_nickname="%s"' % mouse
            & 'lab_name="%s"' % lab
        )
        .proj(
        ...
        .proj("session_date", "session_duration")
        .fetch()
    )
    duration["session_duration_minutes"] = (
        duration.session_duration.dt.total_seconds() / 60
    )
    # convert to minutes
...
def plot_performance_rt(mouse, lab, ax, xlims):
    ...
    behav = pd.DataFrame(
        ...
        .proj("session_date", "performance_easy")
        .fetch(as_dict=True, order_by="session_date")
    )
    ...
    ax.set(
        xlabel="",
        ylabel="Performance (easy trials)",
        xlim=xlims,
        yticks=[0.5, 0.75, 1],
        ylim=[0.4, 1.01],
    )
    ...
    rt = pd.DataFrame(
        (
            (
                behavior_analysis.BehavioralSummaryByDate.ReactionTimeByDate
                * subject.Subject
                * subject.SubjectLab
                & 'subject_nickname="%s"' % mouse
                & 'lab_name="%s"' % lab
        ...
        .proj("session_date", "median_reaction_time")
        .fetch(as_dict=True, order_by="session_date")
    )
    sns.lineplot(
        x="session_date",
        y="median_reaction_time",
        marker="o",
        color="firebrick",
        data=rt,
        ax=righty,
...
def plot_contrast_heatmap(mouse, lab, ax, xlims):
    ...
    session_date, signed_contrasts, prob_choose_right, prob_left_block = (
        (
            behavior_analysis.BehavioralSummaryByDate.PsychResults
            * subject.Subject
            * subject.SubjectLab
            & 'subject_nickname="%s"' % mouse
            & 'lab_name="%s"' % lab
        ...
        .proj(
            "signed_contrasts", "prob_choose_right", "session_date", "prob_left_block"
        )
        .fetch(
            "session_date", "signed_contrasts", "prob_choose_right", "prob_left_block"
        )
    ...
    result = pd.DataFrame(
        {
            "session_date": np.concatenate(session_date),
    ...
    pp2 = result.pivot(
        "signed_contrasts", "session_date", "prob_choose_right"
    ).sort_values(by="signed_contrasts", ascending=False)
...
def fit_psychfunc(df):
    choicedat = (
        df.groupby("signedContrast")
        .agg({"trial": "count", "choice2": "mean"})
        .reset_index()
    )
    pars, L = psy.mle_fit_psycho(
        choicedat.values.transpose(),
        P_model="erf_psycho_2gammas",
        parstart=np.array([choicedat["signedContrast"].mean(), 20.0, 0.05, 0.05]),
        parmin=np.array([choicedat["signedContrast"].min(), 0.0, 0.0, 0.0]),
        parmax=np.array([choicedat["signedContrast"].max(), 100.0, 1, 1]),
    )
    df2 = {
        "bias": pars[0],
        "threshold": pars[1],
...
def plot_psychometric(df, color="black", ax=None, **kwargs):
    ...
    if len(df["signedContrast"].unique()) > 4:
        ...
        pars, L = psy.mle_fit_psycho(
            ...
            P_model="erf_psycho_2gammas",
            parstart=np.array([df2["signedContrast"].mean(), 20.0, 0.05, 0.05]),
            parmin=np.array([df2["signedContrast"].min(), 0.0, 0.0, 0.0]),
            parmax=np.array([df2["signedContrast"].max(), 100.0, 1, 1]),
        )
        sns.lineplot(
            np.arange(-100, 100),
    ...
    if not brokenXaxis:
        ...
        ax.set_xticklabels(
            ["-100", "-25", "-12.5", "-6.25", "0", "6.25", "12.5", "25", "100"],
            size="x-small",
            rotation=-90,
        )
        ax.set_xlim([-40, 40])
