...
def get_weights(mousename, labname):
    ...
    wei["date_time"], wei["weight"] = (
        action.Weighing * subject.Subject * subject.SubjectLab
        & 'subject_nickname="%s"' % mousename
        & 'lab_name="%s"' % labname
    ).fetch("weighing_time", "weight", order_by="weighing_time")
    wei = pd.DataFrame.from_dict(wei)

    # ensure that the reference weight is also added
    restrictions = pd.DataFrame.from_dict(
        (
            action.WaterRestriction * subject.Subject * subject.SubjectLab
            & 'subject_nickname="%s"' % mousename
            & 'lab_name="%s"' % labname
        ).fetch(as_dict=True)
    )
    restr_summary = restrictions[["restriction_start_time", "reference_weight"]].copy()
    restr_summary = restr_summary.rename(
        columns={"restriction_start_time": "date_time", "reference_weight": "weight"}
    )
    ...
    if not wei.empty:
        ...
        wei[
            "days"
        ] = wei.days.dt.days  # convert to number of days from start of the experiment
...
def get_water(mousename, labname):
    ...
    wei = (
        action.WaterAdministration * subject.Subject * subject.SubjectLab
        & 'subject_nickname="%s"' % mousename
        & 'lab_name="%s"' % labname
    ).fetch(as_dict=True)
...
def get_water_weight(mousename, labname):
    ...
    if not (wei.empty or wa.empty):
        ...
        combined = combined[
            ["date", "weight", "water_administered", "water_type", "adlib"]
        ]
        combined["date"] = combined["date"].dt.floor("D")

        # continuous date range - add missing dates to make my life easier later!
        missingdates = np.setdiff1d(
            pd.date_range(combined.date.min(), combined.date.max()), combined["date"]
        )
        combined = pd.merge(
            combined, pd.DataFrame(missingdates, columns=["date"]), how="outer"
        )
        ...
        restrictions["restriction_start_time"] = pd.to_datetime(
            restrictions["restriction_start_time"]
        )
        restrictions["restriction_end_time"] = pd.to_datetime(
            restrictions["restriction_end_time"]
        )

        # round down to the date
        restrictions["date_start"] = restrictions["restriction_start_time"].dt.floor(
            "D"
        )
        restrictions["date_end"] = restrictions["restriction_end_time"].dt.floor("D")

        # fill to the appropriate day
        restrictions["day_start"] = combined["days"].max() * np.ones(
            restrictions["date_start"].shape
        )
        restrictions["day_end"] = combined["days"].max() * np.ones(
            restrictions["date_end"].shape
        )
        ...
        for d in range(len(restrictions)):
            restrictions.loc[d, "day_start"] = datedict[
                restrictions.loc[d, "date_start"]
            ]

            # only do this for dates that are not NaT
            try:
                restrictions.loc[d, "day_end"] = datedict[
                    restrictions.loc[d, "date_end"]
                ]
            except:
                pass

            # for each day, mark if the animal was on WR or not
            combined["water_restricted"] = combined["water_restricted"] | combined[
                "days"
            ].between(
                restrictions["day_start"][d], restrictions["day_end"][d], inclusive=True
            )
...
def get_behavior(mousename, labname, **kwargs):
    ...
    b = (
        ...
        * acquisition.Session.proj("session_end_time", "task_protocol")
    )
    ...
    if not behav.empty:
        ...
        behav["correct"] = np.where(
            np.sign(behav["signedContrast"]) == behav["choice"], 1, 0
        )
        behav.loc[behav["signedContrast"] == 0, "correct"] = np.NaN

        behav["choice2"] = behav.choice.replace(
            [-1, 0, 1], [0, np.nan, 1]
        ...
        behav.loc[
            behav["task_protocol"].str.contains("trainingChoiceWorld"),
            "probabilityLeft_block",
        ] = 0.5
