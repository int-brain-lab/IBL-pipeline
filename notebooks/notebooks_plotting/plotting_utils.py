...
def get_date_range(subj):
    ...
    first_session_date, last_session_date = subj.aggr(
        ...
        last_session_date="max(DATE(session_end_time))",
    ).fetch1("first_session_date", "last_session_date")
    ...
    ).fetch1("first_res_date", "last_res_date")
    ...
    ).fetch1("first_admin_date", "last_admin_date")
    ...
    ).fetch1("first_weighing_date", "last_weighing_date")
    ...
    if last_water_res_date:
        ...
        last_date = np.nanmax(
            [last_session_date, last_water_admin_date, last_weighing_date]
        ) + datetime.timedelta(days=3)
...
def create_status_plot(
    data, yrange, status, xaxis="x1", yaxis="y1", show_legend_external=True
):
