...
from ibl_pipeline import acquisition, behavior, ephys, reference, subject
...
def usage_exit():
    print(
        "usage: {p} [{c}]".format(
            p=os.path.basename(sys.argv[0]), c="|".join(list(actions.keys()))
        )
    )
    sys.exit(0)
...
def shell(*args):
    interact(
        "ibl shell.\n\nschema modules:\n\n  - {m}\n".format(
            m="\n  - ".join(".".join(m.__name__.split(".")[1:]) for m in __all__)
        ),
        local=globals(),
    )
    return 0
