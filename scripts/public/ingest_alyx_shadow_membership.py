...
for key in keys:
    ...
    if not len(action.Surgery & key):
        print("Surgery {} not in the table action.Surgery".format(key["surgery_uuid"]))
        continue

    key_s = dict()
    key_s["subject_uuid"], key_s["surgery_start_time"] = (action.Surgery & key).fetch1(
...
for key in tqdm(keys, position=0):
    ...
    for user in users:
        ...
        if session_user.flush_insert(skip_duplicates=True, chunksz=1000):
...
for key in tqdm(keys, position=0):
    ...
    for procedure in procedures:
        ...
        if session_procedure.flush_insert(skip_duplicates=True, chunksz=1000):
...
keys = (alyxraw.AlyxRaw & sessions_with_projects).proj(session_uuid="uuid")
...
for key in tqdm(keys, position=0):
    ...
    if session_project.flush_insert(skip_duplicates=True, chunksz=1000):
        print("Inserted 1000 session procedure tuples")
