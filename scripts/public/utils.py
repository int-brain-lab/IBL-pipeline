...
from ibl_pipeline.ingest import acquisition, alyxraw
...
def get_uuids(model_name, uuid_name, subject_uuids):
    ...
    if model_name == "subjects.subject":
        return [dict(subject_uuid=subject_uuid) for subject_uuid in subject_uuids]
    else:
        ...
        if "actions." in model_name and model_name != "actions.session":
            uuids = (
                alyxraw.AlyxRaw
                & {"model": model_name}
                & (alyxraw.AlyxRaw.Field & dict(fname="subject") & subjects)
            ).fetch("uuid")
            ...
            for subj_uuid, subj in zip(subject_uuids, subjects):
                mode = dj.config.get("custom", {}).get(
                    "database.mode", os.getenv("MODE", "")
                )
                if mode == "public":
                    session_start, session_end = (
                        public.PublicSubject
                        & (public.PublicSubjectUuid & {"subject_uuid": subj_uuid})
                    ).fetch1("session_start_date", "session_end_date")
                    ...
                    subjects = [
                        dict(subject_uuid=subject_uuid)
            ...
            if model_name == "actions.session":
                uuids = (alyxraw.AlyxRaw & {"model": model_name} & sessions).fetch(
                    "uuid"
                )
                ...
                datasets = [dict(fvalue=str(uuid)) for uuid in dataset_uuids]
                uuids = (
                    alyxraw.AlyxRaw
                    & {"model": model_name}
