...
which cannot be inserted with auto-population, to detect updates.
"""
import json
import uuid
...
from ibl_pipeline.ingest import acquisition, action, alyxraw, data
...
if __name__ == "__main__":
    ...
    for key in keys:
        ...
        user_uuids = grf(key, "users", multiple_entries=True, model="subjects.project")
    ...
    for key in keys:
        ...
        sequences = grf(
            key, "sequences", multiple_entries=True, model="subjects.allele"
        )
        ...
        for sequence in sequences:
            if sequence != "None":
                ...
                key_as["sequence_name"] = (
                    subject.Sequence & dict(sequence_uuid=uuid.UUID(sequence))
    ...
    for key in keys:
        ...
        for allele in alleles:
            if allele != "None":
                key_la = key_l.copy()
                key_la["allele_name"] = (
                    subject.Allele & dict(allele_uuid=uuid.UUID(allele))
                ).fetch1("allele_name")
    ...
    restr_with_users = (
        alyxraw.AlyxRaw.Field & restrictions & 'fname="users"' & 'fvalue!="None"'
    )
    keys = (alyxraw.AlyxRaw & restr_with_users).proj(restriction_uuid="uuid")
    ...
    for key in keys:
        ...
        if not len(action.WaterRestriction & key):
            print(
                "Restriction {} not in the water restriction table".format(
        ...
        key_r["subject_uuid"], key_r["restriction_start_time"] = (
            action.WaterRestriction & key
        ).fetch1("subject_uuid", "restriction_start_time")
    ...
    restrictions = alyxraw.AlyxRaw & 'model = "actions.waterrestriction"'
    restr_with_procedures = (
        alyxraw.AlyxRaw.Field & restrictions & 'fname="procedures"' & 'fvalue!="None"'
    )
    keys = (alyxraw.AlyxRaw & restr_with_procedures).proj(restriction_uuid="uuid")
    ...
    for key in keys:
        ...
        for procedure in procedures:
            ...
            key_rp["procedure_type_name"] = (
                action.ProcedureType & dict(procedure_type_uuid=uuid.UUID(procedure))
            ).fetch1("procedure_type_name")
    ...
    surgeries_with_users = (
        alyxraw.AlyxRaw.Field & surgeries & 'fname="users"' & 'fvalue!="None"'
    )
    keys = (surgeries & surgeries_with_users).proj(surgery_uuid="uuid")

    for key in keys:
        key["uuid"] = key["surgery_uuid"]
        if not len(action.Surgery & key):
            print(
                "Surgery {} not in the table action.Surgery".format(key["surgery_uuid"])
            )
            continue
        ...
        key_s["subject_uuid"], key_s["surgery_start_time"] = (
            action.Surgery & key
        ).fetch1("subject_uuid", "surgery_start_time")
    ...
    surgeries = alyxraw.AlyxRaw & 'model = "actions.surgery"'
    surgeries_with_procedures = (
        alyxraw.AlyxRaw.Field & surgeries & 'fname="procedures"' & 'fvalue!="None"'
    )

    keys = (surgeries & surgeries_with_procedures).proj(surgery_uuid="uuid")
    ...
    for key in keys:
        ...
        if not len(action.Surgery & key):
            print(
                "Surgery {} not in the table action.Surgery".format(key["suggery_uuid"])
            )
            continue
    ...
    other_actions_with_users = (
        alyxraw.AlyxRaw.Field & other_actions & 'fname="users"' & 'fvalue!="None"'
    )
    keys = (other_actions & other_actions_with_users).proj(other_action_uuid="uuid")
    ...
    for key in keys:
        ...
        key_o["subject_uuid"], key_o["other_action_start_time"] = (
            action.OtherAction & key
        ).fetch1("subject_uuid", "other_action_start_time")
    ...
    other_actions = alyxraw.AlyxRaw & 'model = "actions.otheraction"'
    other_actions_with_procedures = (
        alyxraw.AlyxRaw.Field & other_actions & 'fname="procedures"' & 'fvalue!="None"'
    )
    ...
    sessions_with_parents = (
        alyxraw.AlyxRaw.Field & sessions & 'fname="parent_session"' & 'fvalue!="None"'
    )
    keys = (alyxraw.AlyxRaw & sessions_with_parents).proj(session_uuid="uuid")
    for key in keys:
        key["uuid"] = key["session_uuid"]
        if not len(acquisition.Session & key):
            print(
                "Session {} is not in the table acquisition.Session".format(
                    key["session_uuid"]
                )
            )
            continue
        ...
        if not len(acquisition.Session & dict(session_uuid=uuid.UUID(parent_session))):
            print(
                "Parent session {} is not in \
        ...
        key_cs["parent_session_start_time"] = (
            acquisition.Session & dict(session_uuid=uuid.UUID(parent_session))
    ...
    sessions = alyxraw.AlyxRaw & 'model="actions.session"'
    sessions_with_users = (
        alyxraw.AlyxRaw.Field & sessions & 'fname="users"' & 'fvalue!="None"'
    )
    ...
    sessions = alyxraw.AlyxRaw & 'model="actions.session"'
    sessions_with_procedures = (
        alyxraw.AlyxRaw.Field & sessions & 'fname="procedures"' & 'fvalue!="None"'
    )
    ...
    sessions = alyxraw.AlyxRaw & 'model="actions.session"'
    sessions_with_procedures = (
        alyxraw.AlyxRaw.Field & sessions & 'fname="project"' & 'fvalue!="None"'
    )
    keys = (alyxraw.AlyxRaw & sessions_with_procedures).proj(session_uuid="uuid")
    ...
    for key in keys:
        ...
        key_sp["session_project"] = (
            reference.Project & dict(project_uuid=uuid.UUID(project))
        ).fetch1("project_name")
    ...
    admin_with_session = (
        alyxraw.AlyxRaw.Field & admin & 'fname="session"' & 'fvalue!="None"'
    )
    ...
    for key in keys:
        ...
        ).fetch1("subject_uuid", "administration_time")
    ...
    projects = alyxraw.AlyxRaw & 'model="subjects.project"'
    projects_with_repos = (
        alyxraw.AlyxRaw.Field & projects & 'fname="repositories"' & 'fvalue!="None"'
    )
