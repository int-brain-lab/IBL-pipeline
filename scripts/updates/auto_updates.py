...
import ibl_pipeline
from ibl_pipeline import reference, subject, update
...
if __name__ == "__main__":
    ...
    for r in records_for_updating.fetch("KEY"):
        ...
        try:
            ...
            print(
                f'Updated {pk_dict} field "{attribute}" from {original_value} to {updated_value}'
            )
            ...
            UpdateError.insert1(dict(**key, update_error_msg=error_msg))
            print(
                f'Fail to update {pk_dict} field "{attribute}" from {original_value} to {updated_value}'
            )
