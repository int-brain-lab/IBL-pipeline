...
from ibl_pipeline import acquisition, behavior, data
...
if __name__ == "__main__":
    ...
    for key in behavior.CompleteTrialSession.fetch("KEY"):
        try:
            ...
            is_complete = bool(
                np.all([req_ds in datasets for req_ds in required_datasets])
            )
            if is_complete is True:
                if "_ibl_trials.rewardVolume.npy" in datasets:
                    dj.Table._update(
                        behavior.CompleteTrialSession & key,
                        "reward_volume_status",
                        "Complete",
                    )
