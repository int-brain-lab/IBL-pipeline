...
if __name__ == "__main__":
    ...
    from ibl_pipeline.ingest.ingest_alyx_raw import (
        alyxraw,
        get_alyx_entries,
        insert_to_alyxraw,
    )

    insert_to_alyxraw(get_alyx_entries(models=["subjects.subject", "actions.session"]))
    ...
    prob_sessions = (
        acquisition.Session
        - (acquisition.Session.proj(uuid="session_uuid") & session_raw)
    ) & behavior.TrialSet
    ...
    session_start_time_field = dj.U("uuid", "session_start_time") & (
        alyxraw.AlyxRaw.Field & session_raw & 'fname="start_time"'
    ).proj(session_start_time="cast(fvalue as datetime)")
