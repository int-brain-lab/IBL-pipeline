        ...
        for ikey, key in enumerate(trial_sets_go_cue.fetch("KEY")):
            ...
            try:
                ...
                go_cue_times = np.squeeze(
                    ONE().load(eID, dataset_types="_ibl_trials.goCue_times")
                )

                # update go cue time value
                trials = behavior.TrialSet.Trial & key & "trial_go_cue_time is NULL"
    ...
    if len(trial_sets_go_cue_trigger):
        for ikey, key in enumerate(trial_sets_go_cue_trigger.fetch("KEY")):
            try:
                ...
                eID = str((acquisition.Session & key).fetch1("session_uuid"))
                go_cue_times = np.squeeze(
                    ONE().load(eID, dataset_types="_ibl_trials.goCueTrigger_times")
                )

                # update go cue trigger time value
                trials = (
                    behavior.TrialSet.Trial & key & "trial_go_cue_trigger_time is NULL"
                )
                for trial in trials.fetch("KEY"):
                    dj.Table._update(
                        behavior.TrialSet.Trial & trial,
                ...
                if ikey % 100 == 0:
                    print(
                        "Go Cue Trigger entry number: {}/{}".format(
                            ikey, n_go_cue_trigger
                        )
                    )
