...
mismatch_probe_insertions = (
    ...
    & (
        dj.U("subject_uuid", "session_start_time", "probe_idx")
...
for key in tqdm(mismatch_probe_insertions.fetch("KEY")):
    ...
    probe_name, probe_idx = (ephys.ProbeInsertion & key).fetch1(
        "probe_label", "probe_idx"
    )
    ...
    q = (
        ...
        & (
            ...
            & alyx_keys
            & dict(fname="session", fvalue=session_uuid)
        )
        & (alyxraw.AlyxRaw.Field & alyx_keys & dict(fname="name", fvalue=probe_name))
    )

    if len(q) == 1:
        ephys.ProbeInsertion.update1(dict(**key, probe_insertion_uuid=q.fetch1("uuid")))
