...
def update_field(key, eID, trials, alf, djattr, dtype, status, message_record):
    ...
    if data_status != "Missing":
        ...
        if len(dataset) != len(trials):
            message_record.append(dict(**key, error=alf))
        else:
            for itrial, trial_key in enumerate(trials.fetch("KEY")):
                # print(dtype(dataset[itrial]))
                dj.Table._update(
                    behavior.TrialSet.Trial & trial_key, djattr, dtype(dataset[itrial])
                )
    else:
        return
...
if __name__ == "__main__":
    ...
    fields = [
        ...
        {
            "alf": "trials.rewardVolume",
            "djattr": "trial_reward_volume",
            "dtype": float,
            "status": "reward_volume_status",
        },
    ...
    for key in tqdm(keys, position=0):
        try:
            ...
            for field in fields:
                update_field(key, eID, trials, **field, message_record=problematic_keys)
