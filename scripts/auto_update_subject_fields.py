...
if __name__ == "__main__":
    ...
    for subj in tqdm((subject_ingest.Subject).fetch("KEY"), position=0):
        ...
        subj_ingest = subject_ingest.Subject & dict(subject_uuid=subj["subject_uuid"])
        ...
        try:
            sex_new = (
                alyxraw.AlyxRaw.Field & dict(uuid=subj["subject_uuid"], fname="sex")
            ).fetch1("fvalue")
        except Exception:
            continue
        ...
        if sex != sex_new:
            ...
            if sex and sex != "U":
                print(
                    f'updated field "sex" for one entry {subj}, \
                        original: {sex}, new: {sex_new}'
                )

        # ---------------- dob update ------------------
        dob_new = (
            alyxraw.AlyxRaw.Field & dict(uuid=subj["subject_uuid"], fname="birth_date")
        ).fetch1("fvalue")
        ...
        if dob != dob_new:
            ...
            if dob:
                print(
                    f'updated field "birth date" for one entry {subj}, \
                        original: {dob_str}, new: {dob_new_str}'
                )

        # --------------- line update -------------------
        line_uuid = (
            alyxraw.AlyxRaw.Field & dict(uuid=subj["subject_uuid"], fname="line")
        ).fetch1("fvalue")
        ...
        if line_new != line:
            ...
            if line:
                print(
                    f'updated field "line" for one subject {subj}, \
                        original: {line}, new: {line_new}'
                )

        # ---------------- strain update ----------------
        strain_uuid = (
            alyxraw.AlyxRaw.Field & dict(uuid=subj["subject_uuid"], fname="strain")
        ).fetch1("fvalue")
        if strain_uuid == "None":
            strain_new = None
        else:
            # fetch actual strain
            strain_new = (subject_ingest.Strain & dict(strain_uuid=strain_uuid)).fetch1(
                "strain_name"
            )
        ...
        if strain != strain_new:
            ...
            if strain:
                print(
                    f'updated field "strain" for one subject {subj}, \
                        original: {strain}, new: {strain_new}'
                )

        # ----------------- source update ----------------
        source_uuid = (
            alyxraw.AlyxRaw.Field & dict(uuid=subj["subject_uuid"], fname="source")
        ).fetch1("fvalue")
        ...
        if source_uuid == "None":
            ...
            # fetch actual source
            source_new = (subject_ingest.Source & dict(source_uuid=source_uuid)).fetch1(
                "source_name"
            )
        ...
        if source != source_new:
            ...
            if source:
                print(
                    f'updated field "source" for one subject {subj}, \
    ...
    for subj in tqdm(
        (subject.Subject & subject_ingest.Subject.proj()).fetch("KEY"), position=0
    ):
